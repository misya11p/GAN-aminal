# 【コピペOKです】Pythonを使って自動でGoogle画像を収集する方法を徹底解説！ | さすをブログ
...
src = BeautifulSoup(requests.get(url).content, 'html.parser')

images = src.findall('img')
print(src)
...
FILE_NAME = 'dog_'                          # ファイル名（ファイル名の後ろに０からの連番と拡張子が付く）
